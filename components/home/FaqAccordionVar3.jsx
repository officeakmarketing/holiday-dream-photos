export default function FaqAccordionVar3() {
  const faqs = [
    {
  ...
  return (
    <section className="py-16 md:py-24 bg-[#0a150e]">
            ...
            {faqs.map((faq, index) => (
              <details key={index} className="group py-6 sm:py-8 first:pt-0 last:pb-0">
                <summary className="flex w-full items-center justify-between font-heading text-xl sm:text-2xl md:text-3xl text-white cursor-pointer list-none hover:text-brand-red focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-red focus-visible:ring-offset-8 rounded-sm transition-colors duration-300 [&::-webkit-details-marker]:hidden">
                  ...
                  <span className="flex-shrink-0 flex items-center justify-center w-8 h-8 sm:w-10 sm:h-10 rounded-full border border-white/20 text-white group-open:rotate-180 group-open:border-brand-red group-open:bg-brand-red group-open:text-white transition-all duration-300 shadow-sm group-hover:border-brand-red">
